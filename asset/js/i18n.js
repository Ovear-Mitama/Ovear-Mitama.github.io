...
window.SITE_I18N = {
  ...
  en: {
    ...
    /* ---------- Anima:快速开始 ---------- */
    'home.lead': 'A client-side animation library and animation workbench for Minecraft <strong>%mc%</strong>. Add keyframe animation to GUI textures, in-world sprites and text, then arrange them into a full performance on an in-game <strong>multi-lane timeline</strong>.',
    ...
    'home.build.note': '<strong>Requires:</strong> JDK %jdk%. On Windows use <code class="inline">gradlew.bat</code> instead of <code class="inline">./gradlew</code>.',
    'home.run.body': '<code># Fabric client\n./gradlew runClient\n\n# NeoForge client\ncd neoforge &amp;&amp; ./gradlew runClient</code>',
    'home.open.body': '<thead><tr><th>How</th><th>Details</th></tr></thead><tbody><tr><td>The &ldquo;Open Animation Editor&rdquo; key</td><td><strong>Unbound by default</strong> &mdash; bind it under Options &rarr; Controls &rarr; Anima. Already in a world &rarr; opens as a floating window; not in a world &rarr; takes you to the <strong>config world</strong> first, then press the &ldquo;Open Animation Editor&rdquo; key there to toggle the overlay.</td></tr><tr><td>The &ldquo;Play / Pause (editor)&rdquo; key</td><td><strong>Unbound by default</strong> &mdash; pressing Space does the same while the editor has focus.</td></tr><tr><td>Mod Menu</td><td>Mod list &rarr; &ldquo;Anima&rdquo; &rarr; config button.</td></tr><tr><td>From code</td><td><code class="inline">AnimaApi.openTimelineEditor(parent)</code> / <code class="inline">AnimaApi.openAnimationEditor(parent)</code></td></tr></tbody>',
    'home.first.body': '<li>Press the &ldquo;Open Animation Editor&rdquo; key to enter the editor (outside a world it creates and enters the <code class="inline">anima-config</code> sandbox first).</li><li>Drag <strong>Fade In</strong> from the &ldquo;Effects / Particles&rdquo; palette onto the timeline below.</li><li>Drag in <strong>Typewriter In</strong> as well &mdash; entry animations snap to the start of the timeline.</li><li>Drag the playhead on empty timeline space and watch the preview in the middle update live.</li><li>Want particles? Drag in <strong>+ Custom Particle</strong>, then fill in count / spread / speed on the right (particle preview is real 3D particles emitted in the world \u2014 you need to be in the <code class="inline">anima-config</code> world to see it).</li><li>Select a bar and press <kbd>Delete</kbd> to remove it; right-click for the duplicate / group / delete menu.</li>',
  ...
  zhTW: {
    ...
    /* ---------- Anima:快速開始 ---------- */
    'home.lead': 'Minecraft <strong>%mc%</strong> 的客戶端動畫庫與動畫工作台。替 GUI 貼圖、世界內精靈、文字加上關鍵影格動畫，並在遊戲裡用一條<strong>多軌時間軸</strong>把它們編成完整的演出。',
    ...
    'home.build.note': '<strong>需要：</strong>JDK %jdk%。Windows 下把 <code class="inline">./gradlew</code> 換成 <code class="inline">gradlew.bat</code>。',
    'home.run.body': '<code># Fabric 客戶端\n./gradlew runClient\n\n# NeoForge 客戶端\ncd neoforge &amp;&amp; ./gradlew runClient</code>',
    'home.open.body': '<thead><tr><th>方式</th><th>說明</th></tr></thead><tbody><tr><td>「開啟動畫編輯器」按鍵</td><td><strong>預設未指定</strong>，需在「選項 &rarr; 按鍵設定 &rarr; 雜項」裡自行綁定。已經在世界裡 &rarr; 直接開成浮動視窗；不在世界裡 &rarr; 先帶你進入<strong>設定世界</strong>，進去後按「開啟動畫編輯器」按鍵開關浮動視窗。</td></tr><tr><td>「播放 / 暫停（編輯器）」按鍵</td><td><strong>預設未指定</strong>，編輯器取得焦點時按空白鍵效果相同。</td></tr><tr><td>Mod Menu</td><td>模組清單 &rarr;「Anima」&rarr; 設定按鈕。</td></tr><tr><td>程式呼叫</td><td><code class="inline">AnimaApi.openTimelineEditor(parent)</code> / <code class="inline">AnimaApi.openAnimationEditor(parent)</code></td></tr></tbody>',
    'home.first.body': '<li>按「開啟動畫編輯器」按鍵進入編輯器（不在世界裡時會先建立並進入 <code class="inline">anima-config</code> 沙盒世界）。</li><li>從左上的「效果／粒子」把<strong>淡入</strong>拖到下方時間軸。</li><li>再拖一個<strong>打字入場</strong> —— 入場類動畫會自動貼齊時間軸開頭。</li><li>在時間軸空白處拖動播放頭，中間的預覽會即時跟著變。</li><li>想要粒子就拖「<strong>+ 自訂粒子</strong>」，再到右側屬性面板填數量／散佈／速度（粒子預覽是世界裡真實發射的 3D 粒子，需要進入 <code class="inline">anima-config</code> 世界才看得到）。</li><li>選中時間軸上的條按 <kbd>Delete</kbd> 刪除；按右鍵有複製／建立群組／刪除選單。</li>',
